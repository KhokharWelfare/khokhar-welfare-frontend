...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get('https://khokharwelfare-backend.vercel.app/api/admin/donations', { headers: { Authorization: `Bearer ${token}` } }),
      axios.get('https://khokharwelfare-backend.vercel.app/api/admin/users', { headers: { Authorization: `Bearer ${token}` } }),
    ])
  ...
  const handleStatusUpdate = async (id, status) => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.patch(`https://khokharwelfare-backend.vercel.app/api/admin/donations/${id}`, { status }, {
